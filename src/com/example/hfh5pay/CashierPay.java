...
public class CashierPay extends Pay {
	
	public CashierPay(IPayListener listener) {
		super(listener);
	}
	...
	protected void payBefore(Activity activity) {
		TAGS.log("--------------------------收银台支付------------------------------");
		payBeforeSendMessage(PAY_TYPE_CASHIER);
	}
	...
	protected void payDoing(final Activity activity) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String result = HttpUtils.sendPostUTF8(getHost() + "/pay/wap/unifiedorder", params);
				...
				try {
					...
					if("success".equalsIgnoreCase(code)) {
						...
						it.putExtra(DYH5Activity.PAGE_SEE, DYH5Activity.PAGE_CASHIER);
